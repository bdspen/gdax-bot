...
const buy = (coinSymbol, amt) => {
    ...
    .then(({blend: modifier}) => {
        ...
        console.log(`buying with: ${JSON.stringify(buyParams)}`)
    });
};
...
const sell = (coinSymbol, amt) => {
    ...
    analysis.sellSignal(productId).then(signal => {
        if(signal){
            analysis.getTrendPercentModifiers(productId, 6)
            .then(({blend}) => {
        
                amt = getSellAmount(modifier, amt);
                ...
                if (signal >= .5){
                    ...
                    console.log(`selling with: ${JSON.stringify(sellParams)}`) 
               }
...
const coinOn = (coinSymbol, amt, interval) => {
    schedule.scheduleJob(rawInterval('second'), () => {
